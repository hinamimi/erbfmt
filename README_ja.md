...
長い単独のERB tagにも `formatter.lineWidth` を使います。
安全に認識できる単純な command-style Ruby method call は、明示的な括弧を付けて
top-level の引数ごとに展開します。
...
複雑なRuby式、control-flow tag、安全に認識できない式はRuby code自体を保持し、
ERB tagの外枠だけを展開します。
