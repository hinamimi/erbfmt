...
> 公開GitHub Release assetはまだありません。初期releaseはpackage indexや
> extension marketplaceへ登録しません。
...
`v0.1.0` GitHub ReleaseではLinux、macOS、Windows向けprebuilt binary、
platform-specific gem file、download可能なVSIXを提供します。crates.io、npm、
RubyGems.orgには公開しないため、registry経由のinstall commandは現在利用できません。
