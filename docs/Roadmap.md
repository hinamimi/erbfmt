...
- GitHub pre-release `v0.1.0`
- 英語 / 日本語のGitHub Pages documentation
- tag-driven draft release workflow
...
- [Documentation site](https://hinamimi.github.io/erbfmt/): user-facing guide
...
- npm package はまだない。
- Ruby gem はGitHub Release assetとして公開し、RubyGems.orgには登録していない。
- VSCode extension はVSIXをGitHub Releaseで公開し、Marketplaceには登録していない。
...
- tag `v0.1.0`からRelease Binaries workflowを実行し、5jobすべて成功した。
- 4platformのbinary archive、checksum、gemとVSIXの13assetを添付した。
- 4archiveのchecksum一致を確認した。
- Linux x64のstandalone binaryとgemが `erbfmt 0.1.0`を出力することを確認した。
- VSIX metadataが `erbfmt-vscode 0.1.0`であることを確認した。
- GitHub pre-release `erbfmt v0.1.0`を公開した。
- package registryとextension marketplaceには公開していない。

## Milestone 64

Minimal bilingual documentation site

Status: Done

GitHub Pagesで、初めてerbfmtを使う人が必要な情報へ短く到達できるdocs siteを作ります。

やること:

- README相当の概要、install、quick startを英語 / 日本語で用意する。
- `erbfmt.json`の全項目を英語 / 日本語で説明する。
- 公開JSON schemaを同じsiteから取得できるようにする。
- mainへのpushからGitHub Pagesへdeployするworkflowを追加する。
- desktop / mobileの表示とlanguage navigationを確認する。

完了条件:

- OverviewとConfigurationを両言語で閲覧できる。
- schemaに存在するformatter / linter項目が設定ページに揃っている。
- GitHub Pages artifactへstatic filesとschemaが含まれる。
- READMEからdocs siteへ移動できる。

範囲外:

- documentation frameworkやsearch機能の導入
- 大量のguide / tutorial追加
- API documentation自動生成

結果:

- dependencyなしのstatic HTML / CSSとして4ページを追加した。
- 既存VSCode iconをdocs siteのbrand assetとして再利用した。
- responsive navigation、dark color scheme、code exampleを実装した。
- formatter 7項目、linter本体2項目、lint rule 9項目を両言語で説明した。
- GitHub Pages workflowでsiteとJSON schemaをdeployするようにした。
- headless Chromeでdesktop 2画面、mobile 2画面を確認した。
...
次version以降のreleaseで、platform artifactの収集とGitHub draft作成を手作業から
workflowへ移します。

やること:

- `Release Binaries`を手動実行とreusable workflowの両方に対応させる。
- `v*.*.*` tagとrepository内versionの一致をrelease前に検証する。
- 4platformのbinary、checksum、gemとVSIXをexact-nameで収集する。
- checksumを検証し、13asset付きdraft prereleaseを作成する。
- 公開済みreleaseのassetは自動で置き換えない。
- version bump時にuser-facing READMEとdocs siteのversion表記も同期する。

完了条件:

- tag pushからdraft release作成までworkflow定義が揃っている。
- release公開は引き続き人がnotesとassetを確認して実行する。
- malformed tag、version mismatch、asset不足、checksum mismatchで失敗する。
- rerunはdraftだけを更新でき、公開済みreleaseは変更しない。

範囲外:

- GitHub Releaseの自動publish
- package registry / extension marketplaceへのpublish
- release tagの自動作成

結果:

- `.github/workflows/release.yml`を追加した。
- official actionsをNode 24対応majorへ更新した。
- version scriptの同期対象をroot / VSCode / Ruby gem READMEとdocs siteへ拡張した。
- actionlintでrelease workflowとreusable workflowを検証した。
